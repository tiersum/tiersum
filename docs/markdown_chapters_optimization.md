...
**向量索引层面：仍有问题**
1. 父节点 `控制平面`（50 tokens）→ **稀疏向量**
   - 50 token 的文本 → 信息密度低 → HNSW 检索时匹配相关性差
   - vs 500 token 的稠密向量 → 信息丰富 → 检索相关性强
2. 子节点各 500 tokens，本可以贪心组合（如 50 + 500 = 550 ≤ 800），但没有尝试
