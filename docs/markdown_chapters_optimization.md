...
| 2.5 标题识别 | **[已部分实施]** | **Setext**（`===` / `---`）已进解析树；**`N.` 单行大纲**在 remainder **以 ASCII 或全角数字开头**时不再判为标题（减轻 `1. 2. …`、`1. ２…` 类列表误判）。下文旧伪代码 `strings.Contains` 为 **[文档勘误]**。Setext 与「仅 `---`」水平线仍靠「上一行非空作标题」的实用规则区分；**`1. 第一步`** 等仍以汉字开头，属 **[仍开放]**。 |
| 2.2 滑窗与结构 | **[仍开放]** | 超大叶仍 **rune 滑窗**，未做 fence/段落感知切断。 |
| 2.1 / 2.4 合并策略 | **[已澄清]** | `postOrderMergeSplit` 已实现**全量合并贪心**：自底向上、能合则合、不能合则拆开。**无需改为部分合并**（会丢失子标题结构）。 |
...
### 2.1 问题 1：全量合并贪心策略（已澄清为正确行为）

**严重等级**: 🟢 **低**（经讨论，此问题在向量索引和 UI 展示场景下均不严重）
...
**UI 展示层面：不是问题**
- 前端根据文档 ID 查询章节列表，基于 path 重新解析生成章节树
- `Kubernetes 架构/控制平面` 会作为父节点呈现，子章节挂在下方
- 用户看到的是完整树结构，**不存在"孤儿章节"的展示问题**

**向量索引层面：影响有限**
- 父节点只有 50 tokens（标题 + 简介）→ 向量稀疏
- 但父节点**本身无正文内容**，命中与否对检索质量影响不大
- 真正有价值的检索对象是子节点（API Server, etcd, Scheduler）
- 用户查询"Kubernetes API Server"会命中子节点，而非父节点

**为什么不应合并**：
- 如果父节点 + 子节点合并 → path 变成"控制平面"，子标题"API Server"消失
- **改变原始文档结构，不可接受**
- 子节点应保持独立，以保持语义完整性

**算法行为澄清**：

当前实现 **已经是**"自底向上、尽量合并"的贪心策略：
1. 从叶子节点开始，尝试合并自身内容
2. 向上递归，父节点尝试合并**所有子节点 + 自身 localBody**
3. 如果全量合并 <= maxTokens，返回合并结果（贪心成功）
4. 如果全量合并 > maxTokens，保持拆开（贪心失败，回退）

这是正确的"全量合并贪心"策略——在每个节点上，尝试合并**最大可能的章节**（全部子节点），而非选择部分子节点。

**为什么这个策略是正确的**：
- 合并所有子节点 → 保持文档结构完整性（子标题不丢失）
- 如果全量合并失败 → 拆开为独立章节 → 子节点各自编码更精确
- 如果改为部分合并（如只合并前两个子节点）→ 丢失第三个子节点的标题 → 破坏结构

**结论**：当前"全量合并贪心"策略在向量索引和 UI 展示场景下均为最优。无需改为"部分子集贪心"。
...
当前实现 **确实是**"自底向上、尽量合并"：
- 父节点尝试合并**所有子节点 + 自身 localBody**
- 如果全量合并 <= maxTokens，返回合并结果
- 如果全量合并 > maxTokens，拆开为各自独立章节
...
**结论**：当前"全量合并或拆开"策略在向量索引和 UI 展示场景下均可接受。无需改为部分合并。
...
**源码中的实际规则（节选）**：`parseNumberedOutlineHeading` 使用正则 `numberedHeadingSingle` / `numberedHeadingMulti`，并对 remainder 做 `restIsListLike`（以 `*`/`_` 开头等）拒绝；**2026-04-18** 起对 **remainder 首 rune 为 ASCII 数字** 的单行 `N.` 不再判为大纲标题；**全角数字**（`０`–`９`）开头同理。**仍可能**将「`1. 第一步`」等中文步骤判为标题（首字为汉字），属 **[仍开放]**。
...
- **同一合并规则下**：若每层只做「整棵子树 + 本节点 local 能否合成一章」的二元判断，**遍历方向（先序/后序）不改变结果**；优势不在「自顶向下」这个标签本身。
- **「在 ≤ maxTokens 下单章尽量大」**本质是 **标题树约束下的分段/装箱**；一般情形 **没有** 保证全局最优的简单遍历顺序，除非再引入 **显式策略**（例如：对 **有序子节点序列** 做桶式贪心合并、或在超大叶上先做 **段落级装箱** 再滑窗兜底）。
- **段落感知**主要落在 **超大叶** 的兜底阶段（上伪代码 Stage 3 / 对应当前 `splitOversizedRaw` 的替代），与「树合并用先序还是后序」**基本正交**；可在 **保留现有 `postOrderMergeSplit`** 的前提下单独增强。
- **当前实现的「薄父 + 厚子」**来自 **全量合并失败则整段拆开** 的策略，而非「后序必然劣于先序」；若 KPI 仅是 **章体积**，应讨论 **新的局部装箱规则**（及随之而来的 path 语义、golden、冷索引重建），而不是仅把方案叙述改成自顶向下。
...
### 9.2 问题状态汇总（含 UI 展示澄清 + 合并策略澄清）

原列 **6 项** 经重新评估：

| 问题 | 状态 | 说明 |
|------|------|------|
| **#1 全量合并贪心** | 🟢 **已澄清** | 已实现自底向上全量合并贪心；**无需部分合并**（会丢失子标题结构） |
| **#2 滑窗碎代码** | 🔴 **成立** | 向量编码半截代码块，检索质量严重下降 |
| **#3 Token 估算** | 🟡 **已部分实施** | CJK 混合单位已落地，残余风险较低 |
| **#4 爆炸策略激进** | 🟢 **不成立** | 与 #1 同理，保持子节点独立是正确的 |
| **#5 标题识别** | 🟡 **已部分实施** | Setext 与部分编号误判已缓解 |
| **#6 stride** | 🟡 **轻微问题** | 固定 100 token 步长，重叠比例不灵活 |
...
| **P1** | #3 Token 估算 | 🟡 超发 → MiniLM 截断 → 丢失尾部 | 低（已部分实施）|
| **P2** | #6 stride | 🟢 重叠比例影响检索召回 | 低 |
| **P3** | #5 标题识别 | 🟢 结构完整性，间接影响向量 | 低（已部分实施）|
| **-** | #1/#4 合并策略 | 🟢 **不成立**（见 §2.1、§2.4） | - |
...
- **P1（精确 token）**：避免 MiniLM 截断 → 尾部信息不丢失 → 长文档召回 **+15-25%**
- **P2（灵活 stride）**：固定重叠比例 → 检索召回更稳定 → 召回率 **+5-10%**

**注**：以上收益为估算，需通过 A/B 测试验证。**#1/#4 合并策略经评估不成立，不计入收益**。
