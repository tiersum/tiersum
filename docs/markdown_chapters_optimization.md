...
**结论**：当前行为在向量索引和 UI 展示场景下均可接受。父节点的稀疏向量影响有限，无需优化。
...
与问题 1 同理，合并子节点会改变文档结构：
- `第一章 + 1.1 + 1.2` 合并 → path 变成"第一章"，"1.1 小节 A" 消失
- 子标题丢失 → UI 展示不完整
- 向量索引中，子节点独立更符合语义检索需求

**结论**：当前爆炸策略保持子节点独立是正确的，无需优化。
...
| 问题 | 状态 | 说明 |
|------|------|------|
| **#1 缺少部分合并** | 🟢 **不成立** | 合并会改变文档结构（子标题消失），父节点稀疏向量影响有限 |
