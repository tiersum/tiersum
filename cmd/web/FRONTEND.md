...
- **Management** (top bar dropdown after login, `js/components/AppHeader.js`): **Observability** (`/observability`, `/monitoring` redirects here) — all signed-in roles; **Devices & sessions** (`/settings`) — all roles; **Users & API keys** (`/admin`) — **admin** only; **Configuration** (`/admin/config`) — **admin** only, redacted `GET /bff/v1/admin/config/snapshot`. Observability: **Monitoring** tab (health, runtime, cold index stats, Prometheus preview), **Cold probe** (`GET /bff/v1/cold/chapter_hits`, `?tab=cold`), **Traces** (`?tab=traces`).
- **Dark theme**: Slate-style palette
- **Responsive**: Mobile-friendly layout
- **Top bar**: beside **Documents**, **New doc** (`/docs/new`, sm+ screens) opens the create page; the home hero also links **Add a document** / **Browse library**.
