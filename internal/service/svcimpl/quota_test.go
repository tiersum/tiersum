...
func TestQuotaManager_CheckAndConsume(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			qm.usedCount = tt.initialUsed

			result := qm.CheckAndConsume()

			assert.Equal(t, tt.expectedResult, result)
			assert.Equal(t, tt.expectedUsed, qm.usedCount)
		})
	}
}
...
func TestQuotaManager_ResetCycle(t *testing.T) {
	...
	assert.Equal(t, 50, qm.usedCount)

	qm.ForceReset()

	assert.Equal(t, 0, qm.usedCount)
	result := qm.CheckAndConsume()
	assert.True(t, result)
	assert.Equal(t, 1, qm.usedCount)
}
...
func TestQuotaManager_ConcurrentAccess(t *testing.T) {
	...
	assert.Equal(t, 100, qm.usedCount)
}
