...
// Rules follow common operational-security practice: key-based classification with a false-positive allowlist,
// plus conservative value heuristics (JWT, PEM private blocks, common API-key prefixes, URLs with userinfo).
...
// configKeyAllowlist suppresses false positives (e.g. max_tokens vs access_token).
...
func configKeySensitive(key string) bool {
	...
	// Exact leaf names (case-insensitive).
	switch lk {
	...
	// High-confidence suffixes (avoid bare "token" which hits unrelated words).
	suffixes := []string{
	...
	// Bearer / credential-like token carriers (leaf names only).
	...
	if strings.HasSuffix(lk, "_token") && !strings.HasSuffix(lk, "_tokens") && !strings.HasSuffix(lk, "max_tokens") {
		// e.g. bearer_token, subscriber_token; not max_tokens (allowlisted).
		return true
	}

	// Prefix patterns (e.g. secret_key, secret_ref); avoid broad "private_*" (many infra keys are non-secret).
	if strings.HasPrefix(lk, "secret_") {
		return true
	}

	// Composite: …_key only when clearly cryptographic / API-related.
...
func looksLikePEMPrivateBlock(s string) bool {
	return strings.Contains(s, "BEGIN") && strings.Contains(s, "PRIVATE")
}

func looksLikeJWT(s string) bool {
	// Typical JWT: three base64url segments; avoid short random dot-separated IDs.
...
func isLikelyBase64URLSegment(p string) bool {
	...
	// Reject mostly-numeric dotted decimals.
...
func looksLikePrefixedAPISecret(s string) bool {
...
// redactURLUserinfo masks user:password in scheme://user:pass@host URLs (common DSN / JDBC style).
