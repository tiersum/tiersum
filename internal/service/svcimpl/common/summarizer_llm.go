// Package common holds cross-domain helpers used by multiple svcimpl domains (LLM summarization core, quota, config redaction).
package common
...
// SummarizerCore implements LLM-backed document analysis and relevance filtering prompts shared by document ingest and progressive query.
...
// NewSummarizerCore builds a summarizer core with viper-driven summarization limits.
func NewSummarizerCore(provider client.ILLMProvider, logger *zap.Logger) *SummarizerCore {
	...
	if cfg.DocumentSummaryMax == 0 {
		cfg.DocumentSummaryMax = 300
	}
	if cfg.ChapterSummaryMax == 0 {
		cfg.ChapterSummaryMax = 200
	}
	if cfg.ParagraphSummaryMax == 0 {
		cfg.ParagraphSummaryMax = 300
	}

	return &SummarizerCore{
		provider: provider,
		logger:   logger,
		config:   cfg,
	}
...
// AnalyzeDocument runs the full-document LLM analysis JSON prompt.
func (c *SummarizerCore) AnalyzeDocument(ctx context.Context, title string, content string) (*types.DocumentAnalysisResult, error) {
	// First, extract chapters from the document.
	chapters := c.extractChapters(content)
...
Return ONLY the JSON object, no other text.`,
		...
		c.config.DocumentSummaryMax,
		c.config.MaxTagsPerDocument,
		c.config.ChapterSummaryMax,
	)

	metrics.RecordLLMCall(metrics.PathDocAnalyze, EstimateTokens(prompt))

	response, err := c.provider.Generate(ctx, prompt, 4000)
	if err != nil {
		return nil, fmt.Errorf("LLM generation failed: %w", err)
	}

	result, err := c.parseAnalysisResponse(response)
	if err != nil {
		c.logger.Warn("failed to parse LLM response, using fallback", zap.Error(err))
		return c.fallbackAnalysis(title, content, chapters), nil
	}

	c.ensureChapterSummaries(result)

	// Validate and limit tags.
	if len(result.Tags) > c.config.MaxTagsPerDocument {
		result.Tags = result.Tags[:c.config.MaxTagsPerDocument]
	}
...
// FilterDocuments scores documents for a query via LLM JSON array.
func (c *SummarizerCore) FilterDocuments(ctx context.Context, query string, docs []types.Document) ([]types.LLMFilterResult, error) {
	if len(docs) == 0 {
		return nil, nil
	}
	...
	for i, d := range docs {
		docList.WriteString(fmt.Sprintf("[%d] Title: %s\nTags: %v\nSummary: %s\n\n",
			i, d.Title, d.Tags, truncateString(d.Content, c.config.ParagraphSummaryMax)))
	}
...
[
  ...
  ...
	...
	response, err := c.provider.Generate(ctx, prompt, 1500)
	if err != nil {
		c.logger.Error("LLM filter failed", zap.Error(err))
		return c.fallbackFilterDocuments(docs), nil
	}

	return c.parseFilterResults(response), nil
}

// FilterChapters scores chapter rows for a query via LLM JSON array.
func (c *SummarizerCore) FilterChapters(ctx context.Context, query string, chapters []types.Chapter) ([]types.LLMFilterResult, error) {
	if len(chapters) == 0 {
		return nil, nil
	}
	...
	for i, ch := range chapters {
		...
		chapterList.WriteString(fmt.Sprintf("[%d] Path: %s\nSummary: %s\n\n",
			i, ch.Path, truncateString(body, c.config.ParagraphSummaryMax)))
	}
...
// FilterTopicsByQuery scores catalog topics for adaptive narrowing.
func (c *SummarizerCore) FilterTopicsByQuery(ctx context.Context, query string, topics []types.Topic) ([]types.LLMFilterResult, error) {
	if len(topics) == 0 {
		return nil, nil
	}
...
// FilterTagsByQuery scores catalog tags for adaptive filtering.
func (c *SummarizerCore) FilterTagsByQuery(ctx context.Context, query string, tags []types.Tag) ([]types.TagFilterResult, error) {
	if len(tags) == 0 {
		return nil, nil
	}
...
[
  ...
  {"tag": "another-tag", "relevance": 0.82}
	...
	return c.parseTagFilterResults(response), nil
}

func (c *SummarizerCore) extractChapters(content string) []types.ChapterInfo {
...
func (c *SummarizerCore) parseAnalysisResponse(response string) (*types.DocumentAnalysisResult, error) {
...
func (c *SummarizerCore) ensureChapterSummaries(result *types.DocumentAnalysisResult) *types.DocumentAnalysisResult {
	if result == nil {
		return result
	}
	maxLen := c.config.ChapterSummaryMax
	if maxLen < 80 {
		maxLen = 200
	}
...
func (c *SummarizerCore) parseFilterResults(response string) []types.LLMFilterResult {
	jsonStart := strings.Index(response, "[")
	jsonEnd := strings.LastIndex(response, "]")
	if jsonStart == -1 || jsonEnd == -1 || jsonEnd <= jsonStart {
		return nil
...
func (c *SummarizerCore) fallbackAnalysis(title string, content string, chapters []types.ChapterInfo) *types.DocumentAnalysisResult {
	...
	return c.ensureChapterSummaries(&types.DocumentAnalysisResult{
		Summary:  summary,
		Tags:     tags,
		Chapters: chapters,
	})
...
func (c *SummarizerCore) fallbackFilterDocuments(docs []types.Document) []types.LLMFilterResult {
...
func (c *SummarizerCore) fallbackFilterChapters(chapters []types.Chapter) []types.LLMFilterResult {
...
func (c *SummarizerCore) fallbackFilterTopics(topics []types.Topic) []types.LLMFilterResult {
...
func (c *SummarizerCore) parseTagFilterResults(response string) []types.TagFilterResult {
	jsonStart := strings.Index(response, "[")
	jsonEnd := strings.LastIndex(response, "]")
	if jsonStart == -1 || jsonEnd == -1 || jsonEnd <= jsonStart {
		return nil
...
func (c *SummarizerCore) fallbackTagFilter(tags []types.Tag) []types.TagFilterResult {
...
// EstimateTokens approximates token count for metrics (cheap heuristic).
